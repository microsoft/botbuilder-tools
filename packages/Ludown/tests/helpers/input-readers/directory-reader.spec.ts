...
import { directoryReader } from '../../../src/helpers/input-readers/directory-reader';
...
describe('Invalid path validator', () => {
	...
	it('should resolve with the files in the given directory path.', async () => {
		const files = await directoryReader({ extension: '.txt', recursive: true }).read('./sameExt');
	...
	it('should skip the files that dont match the extension given.', async () => {
		const files = await directoryReader({ extension: '.txt', recursive: true }).read('./diffExt');
	...
	it('should not recurse when recursive flag is false.', async () => {
		const files = await directoryReader({ extension: '.txt', recursive: false }).read('./recursive');
	...
	it('should recurse when recursive flag is true.', async () => {
		const files = await directoryReader({ extension: '.txt', recursive: true }).read('./recursive');
