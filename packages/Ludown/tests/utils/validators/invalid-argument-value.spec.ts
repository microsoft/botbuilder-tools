...
describe('Invalid argument value', () => {
	it('should resolve successfully when value given is included in list of allowable values.', async () => {
		const result = await invalidArgumentValueValidatorFactory(['1', '2', '3']).execute({ name: 'option1', value: '1' });

		expect(result).toBeTruthy();
	});

	it('should reject with an error when the value given is not included in the list of allowable values.', async () => {
		try {
	...
	it('should reject with an error when the allowable values array is empty.', async () => {
		try {
	...
	it('should reject with an error when the value to check is undefined.', async () => {
		try {
