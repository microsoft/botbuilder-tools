...
describe('Invalid argument value', () => {
	it('should resolve successfully when value given is included in list of allowable values.', async done => {
		await invalidArgumentValueValidatorFactory(['1', '2', '3'])
			.execute({ name: 'option1', value: '1' })
			.then(result => {
	...
	it('should reject with an error when the value given is not included in the list of allowable values.', done => {
		invalidArgumentValueValidatorFactory(['1', '2', '3'])
			.execute({ name: 'option1', value: '4' })
			.catch(err => {
				expect(err).toEqual({
	...
	it('should reject with an error when the allowable values array is empty.', done => {
		invalidArgumentValueValidatorFactory([])
			.execute({ name: 'option1', value: '1' })
			.catch(err => {
				expect(err).toEqual({
					code: ERROR_CODE.INVALID_ARGUMENT_VALUE,
