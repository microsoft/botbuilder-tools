...
const qnaPromptObj = require('./classes/qnaPrompt')
const parseFileContentsModule = {
    ...
    parseFile: async function (fileContent, log, locale) {
        ...
        //Code to add followup prompts if exits or remove the default prompts array
        try {
            await refactorParsedContent(parsedContent);
        } catch (err) {
            console.log(err)
            throw (err);
        }
        
        //console.log(parsedContent.qnaJsonStructure.qnaList[3])
 ...
 * @returns {parserObj} parsedContent
 * */

const refactorParsedContent = function(parsedContent){
    //console.log("")
    for(var i=0;i<parsedContent.qnaJsonStructure.qnaList.length;i++)
    {
        if(parsedContent.qnaJsonStructure.qnaList[i].hasOwnProperty("prompts")){
            parsedContent.qnaJsonStructure.qnaList[i].context = {
                "isContextOnly": false,
                "prompts": [],
            }
            for(var j=0; j < parsedContent.qnaJsonStructure.qnaList[i].prompts.length;j++)
            {
                let QnAObject = parsedContent.qnaJsonStructure.qnaList.filter(item => item.id === parsedContent.qnaJsonStructure.qnaList[i].prompts[j])[0]
                parsedContent.qnaJsonStructure.qnaList[i].context.prompts.push(new qnaPromptObj(QnAObject,parsedContent))
            }
            //parsedContent.qnaJsonStructure.qnaList[i].context = JSON.stringify(parsedContent.qnaJsonStructure.qnaList[i].context)
            
            
            //console.log(parsedContent.qnaJsonStructure.qnaList[i])
            //console.log("Deleted Prompt")

        }
    }

    //For loop to delete all prompt ids array
    for(var i=0;i<parsedContent.qnaJsonStructure.qnaList.length;i++)
    {
        if(parsedContent.qnaJsonStructure.qnaList[i].hasOwnProperty("prompts")){
        delete parsedContent.qnaJsonStructure.qnaList[i].prompts
        }
    }
    //return parsedContent
}
...
const parseAndHandleIntent = function (parsedContent, chunkSplitByLine) {
    ...
    if (intentName.trim().indexOf(PARSERCONSTS.QNA) === 0) {
        ...
        let QnAId = 0;
        let prompts = []
        //console.log("Now in correct QnA file and function")
        questions.push(intentName.replace('?', '').trim());
        // Added way to recognize and extract QnA Id for updateing existing knowledge base
        ...
        chunkSplitByLine.forEach(function (utterance) {
                ...
                if (utterance.trim().indexOf(PARSERCONSTS.ANSWER) === 0) {
                    ...
                    if (InFiltersSection) {
                        ...
                        if (utterance.includes('@')) {
                            //console.log(utterance.split(' ')[1])
                            QnAId = Number(utterance.split(' ')[1])
                            //console.log(QnAId)
                        }
                        else if((utterance.includes('%')))
                        {
                            prompts.push(Number(utterance.split(' ')[1]))
                        }
                        else
                        {
                            utterance = utterance.slice(1).trim();
            ...
            //console.log(parsedContent.qnaJsonStructure.qnaList[0])
            
        });
        let finalAnswer = answer.substring(0, answer.lastIndexOf(NEWLINE));
        parsedContent.qnaJsonStructure.qnaList.push(new qnaListObj(QnAId, finalAnswer.substring(0, finalAnswer.lastIndexOf('```')).trim(), 'custom editorial', questions, metadata, prompts));
    } else {
