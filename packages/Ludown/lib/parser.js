...
const parser = {
    ...
    handleFile: async function(program, cmd) {
        ...
        try {
            allParsedContent = await parser.parseAllFiles(filesToParse, program.verbose, program.luis_culture);
        } catch (err) {
            throw (err);
        }
    ...
    parseAllFiles : async function(filesToParse, log, luis_culture) {
        ...
        let allParsedContent = new AllParsedContent(allParsedLUISContent, allParsedQnAContent, allParsedAlterationsContent);
        try {
            // resolve utterances deep references
