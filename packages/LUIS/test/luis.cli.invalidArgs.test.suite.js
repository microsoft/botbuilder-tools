...
describe('When the arguments are invalid, the LUIS cli', () => {
    ...
    describe('should print the correct error and display the help contents', () => {
        ...
        it('when the action does not exist', done =>{
            exec(`node ${luis} --authoringKey dummy-key --endpointBasePath dummy-path apps nonexistant`, (error, stdout) => {
                assert.equal(stdout, '');
        ...
        it('when the resource does not exist', done =>{
            exec(`node ${luis} --authoringKey dummy-key --endpointBasePath dummy-path add`, (error, stdout) => {
                assert.equal(stdout, '');
        ...
        it('when the resource is unknown', done =>{
            exec(`node ${luis} --authoringKey dummy-key --endpointBasePath dummy-path add sample`, (error, stdout) => {
                assert.equal(stdout, '');
