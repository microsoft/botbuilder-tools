...
`;
      ...
      assert.strictEqual(activities.length, 6);
      assert.strictEqual(activities[4].attachments.length, 1);
      assert.strictEqual(activities[5].attachments.length, 1);
    });
...
`;
      ...
      const activities = await chatdown(conversation, {});
      const {contentUrl} = activities[5].attachments[0];
      const delimiter = 'base64,';
      assert(contentUrl.includes(delimiter));
      assert(/^([A-Za-z0-9+/]{4})*([A-Za-z0-9+/]{3}=|[A-Za-z0-9+/]{2}==)?$/.test(contentUrl.substr(contentUrl.indexOf(delimiter) + delimiter.length)));
...
bot: Hello, can I help you?`;
      ...
      assert.strictEqual(activities[1].timestamp.toJSON(), new Date(new Date(activities[0].timestamp).getTime() + 2000).toJSON());
    });
...
bot->Joe: Hi Joe!`;
      ...
      const result = await chatdown(conversation);
      assert.strictEqual(result[0].type, 'conversationUpdate', "wrong type 0");
      assert.strictEqual(result[1].type, 'conversationUpdate', "wrong type 1");
      assert.strictEqual(result[2].type, 'conversationUpdate', "wrong type 2");
      assert.strictEqual(result[0].membersAdded[0].name, 'bot', 'bot should be first');
      assert.strictEqual(result[1].membersAdded[0].name, 'Joe', 'Joe should be first');
      assert.strictEqual(result[1].membersAdded[1].name, 'Susan', 'Susan should be second');
      assert.strictEqual(result[2].membersRemoved[0].name, 'Susan', 'Susan should be removed');
    });
...
bot:[Suggestions=option 3|option 4]`;
    ...
    assert.strictEqual(result[1].suggestedActions.actions[0].title, 'option 1');
    assert.strictEqual(result[1].suggestedActions.actions[0].value, 'option 1');
    assert.strictEqual(result[1].suggestedActions.actions[1].title, 'option 2');
    assert.strictEqual(result[1].suggestedActions.actions[1].value, 'option 2');
    assert.strictEqual(result[2].suggestedActions.actions[0].title, 'option 3');
    assert.strictEqual(result[2].suggestedActions.actions[0].value, 'option 3');
    assert.strictEqual(result[2].suggestedActions.actions[1].title, 'option 4');
    assert.strictEqual(result[2].suggestedActions.actions[1].value, 'option 4');
  });
});
