...
import { chatdown } from '../lib';

const pkg = fs.readJsonSync(path.join(__dirname, '../package.json'));
const requiredVersion = pkg.engines.node;
if (!semver.satisfies(process.version, requiredVersion)) {
  process.stderr.write(chalk.bgRedBright`Required node version ${ requiredVersion } not satisfied with current version ${ process.version }.`);
  process.exit(1);
}
 ...
 */
function getInput(args: { [key: string]: any }): Promise<string> {
  if (args._.length > 0) {
    args.in = args._[0];
    return readTextFile.read(path.resolve(args.in));
  } else {
...
async function processFiles(inputDir: string, outputDir: string): Promise<number> {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const fileString = await readTextFile.read(files[i]);
    const activities = await chatdown(fileString, { in: path.dirname(files[i]) });
