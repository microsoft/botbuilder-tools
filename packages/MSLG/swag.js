...
function getActionAndTargetFromName(operationName) {
    let actions = ["get", "create", "update", "delete"];
    let info = {"action": "<action>", "target": "<target>"};
    actions.some(function(element) {
        if(operationName.indexOf(element) >= 0){
            let actionIndexPlusLength = operationName.indexOf(element)+element.length;
            info.action = element;
            info.target = operationName.substr(actionIndexPlusLength, operationName.length -  actionIndexPlusLength).toLowerCase();
            return true;
        }
    });
    return info;
...
// Builds the Service classes and mslg.json
const configsMap = {};
Object.keys(swagger.paths).sort().forEach(pathName => {
    if (pathName.indexOf("languagegeneration") == -1) return;
    const { root, node } = findRootAndNodeFromPath(pathName);
    const fileName = root || node;
    const pathFragment = pathName.substr(pathName.indexOf(fileName) + fileName.length, pathName.length - fileName.length - pathName.indexOf(fileName));
    ...
    while (i--) {
        ...
        const operationName = swaggerOperation.operationId.charAt(0).toLowerCase() + swaggerOperation.operationId.slice(1);
        ...
        // Build the command example for the help output: mslg <api group> <action> <target> <subtarget> --<args>
        let info = getActionAndTargetFromName(operationName);
        let command = `mslg ${info.action} ${info.target}`;
        if (root && root !== node) {
            command += ` ${node} `;
        }
        ...
        // information about this in the mslg.json
        ...
        Object.keys(properties).forEach(propName => {
            ...
            if (propDetails.$ref) {
                ...
                if (type.toLowerCase() != key.toLowerCase())
                    model.imports.push(`const ${type} = require('./${type}');`);
...
// Write the mslg.json
fs.writeJsonSync('lib/api/mslg.json', configsMap);